...
/**
 * Every input below is a real sprint name read from tsgjira.atlassian.net, not
...
describe('shortening a sprint name to its ordinal', () => {
  it('drops a spaced team prefix', () => {
    expect(shortIterationName('CRM TradeBlazers 2026 S18')).toBe('S18');
    expect(shortIterationName('MDS 2026 S17')).toBe('S17');
  });

  it('drops a prefix run together with the year', () => {
    expect(shortIterationName('CRMOPs2026 S18 (08/25-09/08)')).toBe('S18');
  });
  ...
  it('strips a leading zero rather than showing S05', () => {
    expect(shortIterationName('CRM TradeBlazers 2026 S05')).toBe('S5');
  });
  ...
  it('is not fooled by a year that looks like an ordinal', () => {
    expect(shortIterationName('CRM TradeBlazers 2026 S18')).not.toBe('S2026');
  });
});
