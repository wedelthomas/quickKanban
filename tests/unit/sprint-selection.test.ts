 ...
 *
 * Every case here is drawn from board 4200 as it actually is, not from a
...
describe('choosing the iteration from a board that serves two teams', () => {
  it('takes the configured team and ignores the other', () => {
    const mine = sprint();
    const theirs = sprint({ id: 24502, name: 'Signal 2026 S18' });

    expect(selectSprint([mine, theirs], TEAM)?.name).toBe('Anchor Team 2026 S18');
    expect(selectSprint([theirs, mine], TEAM)?.name).toBe('Anchor Team 2026 S18');
  });
  ...
  it('yields nothing when only another team is active', () => {
    expect(selectSprint([sprint({ name: 'Signal 2026 S18' })], TEAM)).toBeNull();
  });

  it('yields nothing for an undated sprint', () => {
    // Real: board 4200's future sprints are named but undated, so a sprint
  ...
  it('reports the ordinal as given, even when it goes backwards', () => {
    ...
    const january = sprint({ id: 25100, name: 'Anchor Team 2027 S1' });
    expect(selectSprint([january], TEAM)?.name).toBe('Anchor Team 2027 S1');
  });
  ...
  it('matches the team name at the start of the sprint name only', () => {
    ...
    const decoy = sprint({ id: 1, name: 'Signal and Anchor Team joint 2026 S18' });
    const real = sprint({ id: 2, name: 'Anchor Team 2026 S18' });
    expect(selectSprint([decoy, real], TEAM)?.id).toBe(2);
  });

  it('ignores surrounding whitespace and case in the configured team name', () => {
    expect(selectSprint([sprint()], '  anchor team ')?.id).toBe(24501);
  });
});
