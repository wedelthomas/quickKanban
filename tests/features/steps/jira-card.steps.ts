 ...
 */
Then(
  'the card for issue {string} has source {string}',
  async function (this: BoardWorld, key: string, source: string) {
    assert.equal((await cardFor(this, key)).source, source);
  },
);

Then(
  'the card for issue {string} links to Jira',
  async function (this: BoardWorld, key: string) {
    const card = await cardFor(this, key);
    assert.ok(
      card.issueUrl?.includes(`/browse/${key}`),
      `expected a browse link, got ${card.issueUrl}`,
    );
  },
);
...
When(
  'the card for issue {string} is deleted',
  async function (this: BoardWorld, key: string) {
    const card = await cardFor(this, key);
...
When(
  'the card for issue {string} is retitled locally to {string}',
...
When(
  'the card for issue {string} is given priority {string} and tags {string}',
  async function (this: BoardWorld, key: string, priority: string, tags: string) {
    const card = await cardFor(this, key);
...
Then(
  'the card for issue {string} has priority {string}',
  async function (this: BoardWorld, key: string, priority: string) {
    assert.equal((await cardFor(this, key)).priority, priority);
  },
);

Then(
  'the card for issue {string} is not on the board',
  async function (this: BoardWorld, key: string) {
...
Then(
  'the card for issue {string} is archived',
  async function (this: BoardWorld, key: string) {
...
Then(
  'the card for issue {string} records why it left',
  async function (this: BoardWorld, key: string) {
...
Then(
  'the last movement for issue {string} was caused by sync',
  async function (this: BoardWorld, key: string) {
...
Then(
  'exactly {int} card exists for issue {string}',
  async function (this: BoardWorld, count: number, key: string) {
...
When(
  '{int} refreshes are requested at once',
...
Then(
  'Jira was queried fewer than {int} times',
  function (this: BoardWorld, ceiling: number) {
    assert.ok(
...
When(
  'the sync interval is set to {int} seconds',
  async function (this: BoardWorld, seconds: number) {
    await this.request('PUT', '/api/settings', { syncIntervalSeconds: seconds });
  },
);

Then(
  'the sync interval is {int} seconds',
  async function (this: BoardWorld, seconds: number) {
    const res = await this.request('GET', '/api/settings');
    assert.equal(
      (res.body as { syncIntervalSeconds: number }).syncIntervalSeconds,
      seconds,
    );
  },
);
...
Then(
  'the card for issue {string} has no history records',
  async function (this: BoardWorld, key: string) {
    const card = await cardFor(this, key);
