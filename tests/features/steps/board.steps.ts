...
When(
  'a card titled {string} is created',
...
Given(
  'a card titled {string} is created with tags {string}',
  async function (this: BoardWorld, title: string, tags: string) {
    await create(this, { title, tags: list(tags) });
  },
);

When(
  'a card titled {string} is created with priority {string}',
  async function (this: BoardWorld, title: string, priority: string) {
...
When(
  'a card titled {string} is created with a due date of today',
  async function (this: BoardWorld, title: string) {
...
When(
  'a card titled {string} is created with a due date of yesterday',
  async function (this: BoardWorld, title: string) {
...
When(
  'a card is created with the title {string}',
  async function (this: BoardWorld, title: string) {
    await create(this, { title });
  },
);
...
When(
  'tags matching {string} are requested',
  async function (this: BoardWorld, q: string) {
...
Then('the columns are in the order {string}', function (this: BoardWorld, order: string) {
  assert.deepEqual(
    board(this).columns.map((c) => c.key),
    list(order),
  );
});
...
Then(
  'the card appears in the {string} column',
...
Then(
  'the card carries exactly the tags {string}',
  function (this: BoardWorld, tags: string) {
...
Then(
  'the request is refused with code {string}',
  function (this: BoardWorld, code: string) {
...
Then('the suggestions include {string}', function (this: BoardWorld, tag: string) {
  ...
  assert.ok(
    body.tags.some((t) => t.name === tag),
    `expected "${tag}" among suggestions`,
  );
});
...
Then(
  'the tag vocabulary contains exactly {int} tag',
