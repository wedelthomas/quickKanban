...
Given(
  'cards titled {string} are created in the {string} column',
  async function (this: BoardWorld, titles: string, columnKey: string) {
...
When(
  'the card is moved to the {string} column at position {int}',
...
When(
  'card {string} is moved to the {string} column at position {int}',
...
When(
  'the card is moved to column {int} at position {int}',
  async function (this: BoardWorld, columnId: number, position: number) {
...
Then('the move is reported as not applied', function (this: BoardWorld) {
  ...
  assert.equal(
    body.moved,
    false,
    'a move to the position already held must not be applied',
  );
});

Then(
  'the card is in the {string} column',
  async function (this: BoardWorld, columnKey: string) {
...
Then(
  'the {string} column holds the cards {string}',
  async function (this: BoardWorld, columnKey: string, titles: string) {
    const board = await fetchBoard(this);
    const column = board.columns.find((c) => c.key === columnKey);
    assert.deepEqual(
      column?.cards.map((c) => c.title),
      list(titles),
    );
  },
);
...
Then(
  'the {string} column positions are contiguous from one',
  async function (this: BoardWorld, columnKey: string) {
    const board = await fetchBoard(this);
    const column = board.columns.find((c) => c.key === columnKey);
    assert.deepEqual(
