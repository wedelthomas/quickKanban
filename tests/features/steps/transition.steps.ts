...
Given('issue {string} offers the transition {string} leading to {string}', function (
  this: BoardWorld,
  key: string,
  name: string,
  toStatus: string,
  ...
  this.jira.setTransitions(key, [{ id: '900', name, toStatusName: toStatus, requiresFields: false }]);
});
...
Given('issue {string} offers only the transition {string} leading to {string}', function (
  this: BoardWorld,
  key: string,
  name: string,
  toStatus: string,
) {
  this.jira.setTransitions(key, [{ id: '902', name, toStatusName: toStatus, requiresFields: false }]);
});

Given('issue {string} offers no transitions at all', function (this: BoardWorld, key: string) {
...
Then('issue {string} has status {string} in Jira', function (
  this: BoardWorld,
  key: string,
  status: string,
) {
...
Then('the card for issue {string} is in conflict', async function (this: BoardWorld, key: string) {
  assert.equal((await cardFor(this, key)).hasConflict, true);
});

Then('the card for issue {string} is not in conflict', async function (
  this: BoardWorld,
  key: string,
) {
...
Then('exactly {int} open conflict exists', async function (this: BoardWorld, count: number) {
...
Then('exactly {int} open conflicts exist', async function (this: BoardWorld, count: number) {
  const { rows } = await this.pool.query<{ count: string }>(
    'SELECT count(*) FROM conflicts WHERE resolved_at IS NULL',
  );
  assert.equal(Number(rows[0]!.count), count);
...
Then('the conflict shows Jira as {string}', async function (this: BoardWorld, status: string) {
...
Given("issue {string}'s last recorded status is {string}", async function (
  this: BoardWorld,
  key: string,
  status: string,
) {
