...
if [[ -f "$IMPL_PLAN" ]]; then
    ...
    if [[ -n "$TEMPLATE" ]] && [[ -f "$TEMPLATE" ]]; then
        # BEGIN tradestation-sdd (composition)
    _sdd_render="$REPO_ROOT/.specify/presets/tradestation-sdd/scripts/render-template.sh"
    if _sdd_c=$("$_sdd_render" "plan-template" "$REPO_ROOT" 2>/dev/null) && [ -n "$_sdd_c" ]; then
        printf '%s' "$_sdd_c" > "$IMPL_PLAN"
    else
        cp "$TEMPLATE" "$IMPL_PLAN"
    fi
    # END tradestation-sdd (composition)
        if $JSON_MODE; then
