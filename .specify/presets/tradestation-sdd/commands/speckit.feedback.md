---
description: "Capture developer feedback on the SDD workflow and post it to the centralized feedback tracker"
---
...
7. **Also write a sibling JSON payload** next to the `.md`, named
   ...
   ```jsonc
   {
     ...
     "tool": "claude",                  // originating AI tool
   ...
   `{"input": <n>, "output": <n>, "cache_creation": <n>, "cache_read": <n>}` of
   numbers (see the `session_cost` field below), never its `input=…, output=…`
   string rendering. The backend only counts token usage when `session_cost` is
   an object; the string form is silently dropped.
8. Invoke the telemetry client with the JSON payload:
   ...
   **First-time auth is automatic.** On the very first run with no cached
   token, the client opens your browser once to authorize (instant if you are
   already signed in to the portal), caches the token, and continues — no
   manual step. It prompts at most once; if you dismiss it, or you are on a
   headless/CI box with no browser, telemetry just queues and the client tells
   you to run `bash .specify/presets/tradestation-sdd/scripts/post-feedback.sh --auth`
   (which also handles the paste-back flow on headless boxes). Re-auth on token
   expiry (~90 days) is the same one-time `--auth` command. Queued items flush
   automatically once authorized.
...
- `session_minutes` — wall-clock minutes from the oldest message in
  the current Claude Code JSONL session to now, one decimal:
  ```bash
...
- `feature_age_hours` — oldest mtime under `specs/<feature-id>/` to
  ...
  ```
- `session_cost` — sum `usage` fields across the current Claude Code
  session transcript:
  ```bash
  proj_dir="$HOME/.claude/projects/$(pwd | sed 's|/|-|g')"
  jsonl="$(ls -t "$proj_dir"/*.jsonl 2>/dev/null | head -1)"
  ...
  In the JSON payload (step 7), `metrics.session_cost` MUST be that raw jq
  object — `{"input": <n>, "output": <n>, "cache_creation": <n>, "cache_read":
  <n>}` with numeric values — **not** the string rendering: the backend only
  sums token usage when `session_cost` is an object and silently drops a string.
  If `jq` or the file is missing, render `unknown` in the markdown and omit
  `session_cost` from the JSON `metrics`.

  This is a Claude Code internal path. If it changes upstream, only
  this field breaks; the rest of telemetry is git/filesystem-only.
- `test_files_count`, `test_to_source_ratio`, `coverage_report_present`
  — see **Test coverage proxies** below.
- `spec_modified_after_tasks`, `needs_clarification_added_during_impl`,
  `acceptance_scenarios`, `frs_unreferenced` — see **Spec drift
...
self-report; these fields capture what Claude Code's session transcript
...
Same Claude-Code internal path as `session_cost`. If Claude Code's
JSONL format changes upstream, only these fields break; everything
else is git/filesystem-only. Render every field as `unknown` if `jq`
or the JSONL is missing.
...
jq -s '
  ...
  | { calls: length,
      files: ([.[].input.file_path] | unique | length),
      lines: ([.[]
        | if   .name == "Write"        then (.input.content    | split("\n") | length)
          elif .name == "NotebookEdit" then (.input.new_source | split("\n") | length)
          elif .name == "Edit"         then (
...
- `ai_edit_tool_calls` — `.calls` from the jq output. Count of
  `Edit` + `Write` + `NotebookEdit` tool uses by the agent in the
  current session.
...
- `ai_lines_added_estimate` — `.lines`. Net new lines from agent tool
  calls. For Edit, computed as `max(0, new_string_lines − old_string_lines)`,
  so it does not count line-for-line replacements. For Write, the full
  content (overcounts when overwriting an existing file). Best-effort.
...
the values):
- **Claude Code only.** Code typed by hand, accepted from
  Cursor/Copilot, or pasted from web Claude is *not* counted as AI
  here. The attestation prompt is the catch-all for those.
...
- **Multi-session features.** A session is one Claude Code run; a
  ...
  only the final session unless we widen the JSONL scan — out of
