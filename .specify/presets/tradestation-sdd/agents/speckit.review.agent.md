---
description: Independent second-pass code review of the working branch diff vs main. Writes code-review.md with blocking and non-blocking findings.
---
...
4. **Invoke complementary Copilot tools (best-effort, non-blocking).**
   Invoke complementary Copilot tools if available (e.g., built-in
   `/review` skill). If unavailable, note it and continue. Fold any
   findings into the review under a `## Skill output` section.
   Complementary tools augment this command; they are not required.
...
6. **Print blocking-finding count to stdout** along with the path to
   ...
   invocation scoped to the findings)
   addresses them.
