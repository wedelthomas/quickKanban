...
1. Run the deterministic checker and read its JSON:
   `bash .specify/presets/tradestation-sdd/scripts/doctor.sh --json --tool copilot`
   It returns checks R1 (model — Opus **4.6 or later**, e.g. `claude-opus-4.6`),
   R2 (effort `high`), R3 (superpowers plugin), R5 (Spec-Kit initialized), R6
   (preset installed), and R7 (portal telemetry authentication). Under
   `--tool copilot`, R1/R2 read `~/.copilot/settings.json` and R3 checks for a
   superpowers Copilot plugin. R1–R3, R5, R6 are hard PASS/FAIL; R7 is advisory.
...
3. **R4 — Closed-loop testing capability** (judgment check, not in the script).
   ...
   Inspect available tools and project configuration. If none is found, ask the
   user the project type, then recommend the matching tool.
