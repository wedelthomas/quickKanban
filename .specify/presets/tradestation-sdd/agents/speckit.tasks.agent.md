...
4. **Generate tasks.md**: Use the composed tasks template from `bash .specify/presets/tradestation-sdd/scripts/render-template.sh tasks-template` (its stdout) as structure, fill with:
...
**Tests are MANDATORY** when spec defines coverage, success criteria reference CI/tests, plan lists test files, or constitution mandates tests. Tests appear BEFORE implementation tasks (test-first).
