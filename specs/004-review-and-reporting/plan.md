...
| Failure modes | Interrupted / abandoned work | **Single-flight needs someone to clear the row when a process dies holding it.** `archive_runs` uses a partial unique index on "unfinished", which is the right guarantee while a pass runs and a trap once one crashes: `finished_at` stays null and every later pass violates the index. `abandonUnfinished()` sweeps orphaned rows at startup, marking them failed rather than deleting them. Added after review found it missing — the table shape was copied from `sync_runs` and its recovery was not. |
